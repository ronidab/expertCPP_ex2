...
int System::portExist(string &other_port_name) const {
    ...
    for (auto p :ports_dictionary) {
        if (!p.second.getPortName().compare(other_port_name)) { return p.first; }
    }
    return -1;
}
...
void System::printGraph(Graph &g) {
    for (int i = 0; i < g.getSize(); i++) {
        ...
        g.graph[i].setToHead();
        while (!g.graph[i].endOfLinkedList()) {
            cout << " ---" << g.graph[i].getCursor().lock().get()->data.distance_or_capacity << "---> ";
            cout << ports_dictionary.find(g.graph[i].getCursor().lock().get()->data.port.getID())->second.getPortName()
                 << " ";
            ++g.graph[i];
        }
        cout << endl;
...
void System::printTimesGraph() { printGraph(distanceGraph); }

void System::printContainersGraph() { printGraph(containersGraph); }
